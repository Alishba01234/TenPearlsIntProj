...
# FETCH + ENGINEER A SHORT RECENT WINDOW
...
def select_rows_to_upsert(engineered_df: pd.DataFrame, latest_stored_dt) -> pd.DataFrame:
    """Hopsworks/Hudi upserts on the datetime primary key 
    (see this module's docstring), so resending an already-stored datetime 
    with a now-computable target simply overwrites the stale NaN in place.
...
def align_dtypes_to_feature_group(df: pd.DataFrame, fg) -> pd.DataFrame:
    ...
    exactly, read live from Hopsworks rather than assumed. pandas'/numpy's 
    default integer dtype is platform-dependent -- int64 on Linux/Mac, int32 
    on Windows. The feature group's schema was locked in by whichever platform 
    ran the very first insert (via aqiPipeline.py). Any later insert -- like 
    this hourly one -- run from a DIFFERENT platform can produce a different
...
# PRUNE: KEEP ONLY THE LATEST N YEARS
...
# MIRROR THE SAME APPEND + PRUNE ON THE LOCAL CSV
